...
class CommentService
{
    ...
    {
        $commentRating = $this->commentRatingRepository->findOneBy([
            'user'    => $user, 
            'comment' => $comment,
        ]);

        if (! empty($commentRating)) {
            $this->removeLike($commentRating, $comment, $flush);
     ...
     * @return bool Returns true if like added
     */
    private function removeLike(CommentRating $commentRating, Comment $comment, bool $flush = true)
    {
        $comment->setRating($comment->getRating() - 1);
        $this
            ->commentRatingRepository
            ->remove($commentRating, $flush);
